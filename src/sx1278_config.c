...
{
    return spi_burst_read_reg(sx127x_spi, 0x12, data, 1);
}

sx127x_err_t sx1278_clear_irq()
{
    uint8_t data = 0xFF;
    return spi_burst_write_reg(sx127x_spi, 0x12, &data, 1);
}
...
{
    return spi_burst_write_reg(sx127x_spi, 0x01, &mode_register, 1);
}

sx127x_err_t sx_1278_get_op_mode(uint8_t *data)
{
    return spi_burst_read_reg(sx127x_spi, 0x01, data, 1);
}
...
sx127x_err_t sx_1278_switch_to_nth_channel(size_t n)
{
    ...
    ret = spi_burst_write_reg(sx127x_spi, 0x06, &data, 1); // send frf big endian
    if (ret != SX_OK)
        goto cleanup;

    data = (frf >> 8) & 0xFF;
    ret = spi_burst_write_reg(sx127x_spi, 0x07, &data, 1);
    if (ret != SX_OK)
        goto cleanup;

    data = (frf) & 0xFF;
    ret = spi_burst_write_reg(sx127x_spi, 0x08, &data, 1);
...
sx127x_err_t sx_1278_set_spreading_factor(uint8_t sf)
{
    ...
    data = 0b00000100;
    data |= sf << 4;
    ret = spi_burst_write_reg(sx127x_spi, 0x1E, &data, 1);
    if (ret == SX_OK)
        spreading_factor = sf;
...
sx127x_err_t sx1278_set_bandwidth(bandwidths bw, coding_rate cr, bool enable_explicit_headers)
{
    ...
    if (!enable_explicit_headers)
        ...
        data &= ~0x01;
    ret = spi_burst_write_reg(sx127x_spi, 0x1D, &data, 1);
    if (ret == SX_OK)
    {
...
sx127x_err_t initialize_sx_1278()
{
    ...
    data = 0x00;
    ret = spi_burst_write_reg(sx127x_spi, 0x0D, &data, 1); // set fifo ptr addr
    if (ret != SX_OK)
        return ret;

    ret = spi_burst_write_reg(sx127x_spi, 0x0F, &data, 1); // set rx fifo base addr
    if (ret != SX_OK)
        return ret;

    ret = spi_burst_write_reg(sx127x_spi, 0x0E, &data, 1); // set tx fifo base baddr
    if (ret != SX_OK)
        return ret;
    ...
    ret = spi_burst_write_reg(sx127x_spi, 0x09, &data, 1); // power
    if (ret != SX_OK)
        return ret;

    data = 0b00101011;                                     // max current 100mA
    ret = spi_burst_write_reg(sx127x_spi, 0x2B, &data, 1); // OCB
    if (ret != SX_OK)
        return ret;

    data = 0x84;
    ret = spi_burst_write_reg(sx127x_spi, 0x4D, &data, 1); // extra power mode not set
    if (ret != SX_OK)
        return ret;

    data = 0b01110010;
    ret = spi_burst_write_reg(sx127x_spi, 0x1D, &data, 1); // 125kHz 4/5 coding explicit headers
    if (ret != SX_OK)
        return ret;
    ...
    data = 0xFF;
    ret = spi_burst_write_reg(sx127x_spi, 0x1F, &data, 1); // 0xFF symbtimeout
    if (ret != SX_OK)
        return ret;

    data = 0x00;
    ret = spi_burst_write_reg(sx127x_spi, 0x20, &data, 1); // preamble length msb and lsb
    if (ret != SX_OK)
        return ret;

    data = 0x18;
    ret = spi_burst_write_reg(sx127x_spi, 0x21, &data, 1);
    if (ret != SX_OK)
        return ret;

    data = SFD;
    ret = spi_burst_write_reg(sx127x_spi, 0x39, &data, 1); // sync word
    if (ret != SX_OK)
        return ret;
