...
sx127x_err_t sx1278_poll_and_read_packet(packet *rx_p, int timeout)
{
    ...
    if (ret != SX_OK)
    {
        network_log_err(&sx127x_logger, TAG, "couldnt switch to rx single");
        goto cleanup;
    }
    ...
    if (ret != SX_OK)
    {
        network_log_err(&sx127x_logger, TAG, "couldn't poll for packet received flag, got. ");
        goto cleanup;
    }
...
sx127x_err_t sx_1278_send_packet(packet *p, int switch_to_rx_after_tx)
{
    ...
    network_log_with_tag(&sx127x_logger, TAG, "sending packet", LOG_INFO_LOW);
    free(p_desc);
    ...
    if (packet_size == -1)
    {
        network_log_err(&sx127x_logger, TAG, "couldnt convert packet to bytesteam\n");
...
static sx127x_err_t switch_to_fsk()
{
    ...
    if (ret != SX_OK)
    {
        network_log_err(&sx127x_logger, TAG, "couldnt set sleep mode on lora\n");
        return ret;
    }
    ...
    if (ret != SX_OK)
    {
        network_log_err(&sx127x_logger, TAG, "couldnt set sleep mode on fsk\n");
        return ret;
    }
    ...
    if (ret != SX_OK)
    {
        network_log_err(&sx127x_logger, TAG, "couldnt read operation mode register");
        return ret;
    }
    if (data >> 7)
    {
        network_log_err(&sx127x_logger, TAG, "operational mode stuck at lora ");
        return ret;
    }
...
sx127x_err_t send_packet_ensure_ack(packet *p, int timeout, packet_types ack_type)
{
    ...
    for (int i = 0; i < timeout_n; i++)
    {
        ...
        if (ret != SX_OK)
        {
            network_log_err(&sx127x_logger, TAG, "error occured while sending the packet");
            goto cleanup;
        }
        ...
        if (ret != SX_OK)
        {
            network_log_with_tag(&sx127x_logger, TAG, "error occured while reading packet. retrying...", LOG_INFO_LOW);
            continue;
        }
        ...
        if (!check_packet_features(rx_p, p->dest_address, p->src_address, p->ack_id, p->sequence_number, ack_type))
        {
            ...
            network_log_with_tag(&sx127x_logger, TAG, "mismatched packet. retrying...", LOG_INFO_LOW);
            continue;
        }

        network_log_with_tag(&sx127x_logger, TAG, "ack accepted", LOG_INFO_LOW);
...
sx127x_err_t sx_1278_get_channel_rssis(double *rssi_data, size_t *len)
{
    ...
    if (ret != SX_OK)
    {
        network_log_err(&sx127x_logger, TAG, "couldnt switch to fsk mode\n");
        return ret;
    }
    ...
    if (ret != SX_OK)
    {
        network_log_err(&sx127x_logger, TAG, "couldnt set rx mode on fsk\n");
        return ret;
    }
    ...
    for (size_t i = 0; i < channel_n; i++)
    {
        ...
        {
            if (i == channel_n - 1)
                network_log_err(&sx127x_logger, TAG, "couldnt switch to channel");
            else
                network_log_err(&sx127x_logger, TAG, "couldnt switch to channel, trying next one");
            continue;
        }

        sx127x_task_delay_ms(&sx127x_timer_ctx, RSSI_READ_DELAY_MS);

        ret = spi_burst_read_reg(&sx127x_spi_port, 0x11, &ret_data, 1);
        if (ret != SX_OK)
        {
            if (i == channel_n - 1)
                network_log_err(&sx127x_logger, TAG, "couldnt read rssi on channel.");
            else
                network_log_err(&sx127x_logger, TAG, "couldnt read rssi on channel. switching to next channel");
            continue;
        }
    ...
    *len = channel_n;
    ...
    if (ret != SX_OK)
    {
        network_log_err(&sx127x_logger, TAG, "couldnt switch back to lora sleep mode");
    }
    return ret;
}
